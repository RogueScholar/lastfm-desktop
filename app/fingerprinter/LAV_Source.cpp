...
public:
  LAV_SourcePrivate()
      : inFormatContext(NULL), inCodecContext(NULL)
#if defined(HAVE_SWRESAMPLE) || defined(HAVE_AVRESAMPLE)
        ,
        resampleContext(NULL)
#endif
        ,
        streamIndex(-1), duration(0), timestamp(0), bitrate(0), eof(false),
        overflowSize(0) {
 ...
 */
uint8_t *LAV_SourcePrivate::decodeOneFrame(int &dataSize, int &channels,
                                           int &nSamples) {
  char buf[256];
  AVPacket packet;
  ...
  while (!frameFinished) {
    ...
    ret = avcodec_decode_audio4(inCodecContext, decodedFrame, &frameFinished,
                                &packet);
    if (ret < 0) {
      av_strerror(ret, buf, sizeof(buf));
    ...
    if (frameFinished) {
      ...
      if (!resampleContext && (inCodecContext->sample_fmt != outSampleFmt ||
                               inCodecContext->channels != channels)) {
        int64_t inChannelLayout =
            (inCodecContext->channel_layout &&
             inCodecContext->channels == av_get_channel_layout_nb_channels(
                                             inCodecContext->channel_layout))
                ? inCodecContext->channel_layout
        ...
        resampleContext = swr_alloc_set_opts(
            NULL, outChannelLayout, outSampleFmt, decodedFrame->sample_rate,
            inChannelLayout, inCodecContext->sample_fmt,
            inCodecContext->sample_rate, 0, NULL);
        if (!resampleContext || swr_init(resampleContext) < 0) {
          cerr << "Cannot create sample rate converter from "
               ...
               << inCodecContext->sample_rate << " Hz "
               << av_get_sample_fmt_name(outSampleFmt) << " " << channels
               << " channels." << endl;
          av_free_packet(&packet);
          break;
        }
      ...
      if (resampleContext) {
        ...
        int nSamplesOut = swr_convert(
            resampleContext, &outBuffer, maxOutSamples,
      ...
      if (!resampleContext && (inCodecContext->sample_fmt != outSampleFmt ||
        ...
        av_opt_set_int(resampleContext, "in_channel_layout", inChannelLayout,
                       0);
        av_opt_set_int(resampleContext, "in_sample_fmt",
                       inCodecContext->sample_fmt, 0);
        av_opt_set_int(resampleContext, "in_sample_rate",
                       inCodecContext->sample_rate, 0);
        av_opt_set_int(resampleContext, "out_channel_layout", outChannelLayout,
                       0);
        ...
        if (av_get_bytes_per_sample(inCodecContext->sample_fmt) <= 2 &&
          ...
          av_opt_set_int(resampleContext, "internal_sample_fmt",
      ...
      if (resampleContext) {
        int outLinesize;
        int maxOutSamples = outBufferSize / channels / outSampleSize;
        av_samples_get_buffer_size(&outLinesize, channels,
                                   decodedFrame->nb_samples, outSampleFmt, 0);
        int nSamplesOut = avresample_convert(
            resampleContext, &outBuffer, outLinesize, maxOutSamples,
            decodedFrame->extended_data, decodedFrame->linesize[0],
            decodedFrame->nb_samples);
        if (nSamplesOut < 0) {
    ...
    if (packet.pts != AV_NOPTS_VALUE)
      timestamp =
          av_q2d(inFormatContext->streams[streamIndex]->time_base) * packet.pts;
    av_free_packet(&packet);
  }
...
LAV_Source::LAV_Source() : d(new LAV_SourcePrivate()) {
  av_register_all();
...
void LAV_Source::init(const QString &fileName) {
  ...
  if (avformat_open_input(&d->inFormatContext, QFile::encodeName(fileName),
                          NULL, NULL) < 0) {
  ...
  AVCodec *codec;
  int videoIndex = av_find_best_stream(d->inFormatContext, AVMEDIA_TYPE_VIDEO,
                                       -1, -1, NULL, 0);
  if (videoIndex < 0)
    videoIndex = -1;
  d->streamIndex = av_find_best_stream(d->inFormatContext, AVMEDIA_TYPE_AUDIO,
                                       -1, videoIndex, &codec, 0);
  ...
  if (duration < 1) {
    ...
    AVRational timeBase =
        d->inFormatContext->streams[d->streamIndex]->time_base;
  ...
  if (!avcodec_open2(d->inCodecContext, codec, NULL) < 0) {
    ...
    throw std::runtime_error(
        "Unable to open a compatible audio codec for the file!");
  }
  ...
  if (d->inCodecContext->sample_fmt != outSampleFmt ||
      d->inCodecContext->channels < 1 || d->inCodecContext->channels > 2) {
    release();
...
void LAV_Source::getInfo(int &lengthSecs, int &samplerate, int &bitrate,
                         int &nchannels) {
