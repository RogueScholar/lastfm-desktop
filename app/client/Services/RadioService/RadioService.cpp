...
RadioService::RadioService()
    : m_audioOutput(0), m_mediaObject(0), m_state(Stopped),
      m_bErrorRecover(false), m_maxUsageCount(180) {
  initRadio();
  ...
  connect(aApp, SIGNAL(sessionChanged(unicorn::Session)),
          SLOT(onSessionChanged(unicorn::Session)));
}

void RadioService::onSessionChanged(const unicorn::Session &session) {
...
void RadioService::onLastFmUrl(const QUrl &url) {
...
// todo:
// note:
// if the station is the same as current station (ie: the user hit stop then
// start) then we *don't* retune.  norman is quite emphatic about this.  :)
void RadioService::play(const RadioStation &station) {
  if (!aApp->currentSession().youRadio()) {
    // they are not a subscriber so don't let them start the radio
    emit error(lastfm::ws::SubscribersOnly,
               tr("You need to be a subscriber to listen to radio"));
    return;
  }
  ...
  if (m_station.url() == "") {
    ...
    QString stationTitle =
        us.value("lastStationTitle", tr("A Radio Station")).toString();
  ...
  m_station.setMainstr(
      unicorn::AppSettings().value("mainstr", 0.5).toDouble(&ok));
  ...
  connect(m_tuner, SIGNAL(error(lastfm::ws::Error, QString)),
          SLOT(onTunerError(lastfm::ws::Error, QString)));

  changeState(TuningIn);
}

// play this radio station after the current track has finished
void RadioService::playNext(const RadioStation &station) {
  if (m_state == Playing) {
    m_station = station;
...
void RadioService::skip() {
  ...
  if (q.size()) {
    ...
    // if the error returns a 403 permission denied error, the mediaObject is
    // uninitialised
    if (m_mediaObject)
      m_mediaObject->play();
    else {
    ...
    m_mediaObject->blockSignals(
        true); // don't tell outside world that we stopped
    m_mediaObject->stop();
...
void RadioService::onTunerError(lastfm::ws::Error e, const QString &message) {
...
void RadioService::stop() {
  ...
  m_mediaObject->blockSignals(
      true); // prevent the error state due to setting current source to null
  m_mediaObject->stop();
...
void RadioService::clear() {
  m_track = Track();
  m_station = RadioStation();
  delete m_tuner;
}

void RadioService::volumeUp() {}

void RadioService::volumeDown() {}

void RadioService::mute() {
  m_audioOutput->setMuted(!m_audioOutput->isMuted());
}

void RadioService::onPhononStateChanged(Phonon::State newstate,
  ...
  case Phonon::ErrorState:
    ...
    emit error(lastfm::ws::UnknownError,
               QVariant(m_mediaObject->errorString()));
    deInitRadio();
    changeState(Stopped);
    break;
...
void RadioService::restoreVolume() {
  ...
  m_audioOutput->setMuted(
      unicorn::AppSettings().value("Muted", false).toBool());
}

void RadioService::phononEnqueue() {
...
void RadioService::onPhononCurrentSourceChanged(const Phonon::MediaSource &) {
  if (m_mediaObject == 0) {
    qDebug() << "m_mediaObject is null!";
    return;
  }
...
void RadioService::changeState(State const newstate) {
...
void RadioService::setStationName(const QString &s) {
...
void RadioService::setSupportsDisco(bool supportsDiscovery) {
  emit supportsDisco(supportsDiscovery);
}

void RadioService::setMaxUsageCount(int count) { m_maxUsageCount = count; }

void RadioService::onBuffering(int percent_filled) {
  Q_UNUSED(percent_filled);
  emit buffering(percent_filled);
}

void RadioService::onMutedChanged(bool muted) {
  unicorn::AppSettings().setValue("Muted", muted);
}

void RadioService::onOutputDeviceChanged(
    const Phonon::AudioOutputDevice &newDevice) {
  qDebug() << "name: " << newDevice.name()
           << " description: " << newDevice.description();
}

void RadioService::onVolumeChanged(qreal vol) {
  unicorn::AppSettings().setValue("Volume", vol);
}

void RadioService::onFinished() {
...
bool RadioService::initRadio() {
  ...
  qDebug() << (audioOutput->isMuted() ? "muted," : "not-muted,")
           ...
           << audioOutput->volumeDecibel() << "db " << audioOutput->volume();
  foreach (QByteArray a, audioOutput->outputDevice().propertyNames()) {
    qDebug() << a << ":" << audioOutput->outputDevice().property(a);
  }

  QString audioOutputDeviceName =
      ""; // TODO moose::Settings().audioOutputDeviceName();
  ...
  if (!m_path.isValid()) {
    ...
    // can't delete the mediaObject after a failed Phonon::createPath without a
    // crash in phonon...  (Qt 4.4.3) so, we leak it:
  ...
  connect(mediaObject, SIGNAL(stateChanged(Phonon::State, Phonon::State)),
          SLOT(onPhononStateChanged(Phonon::State, Phonon::State)));
  connect(mediaObject, SIGNAL(bufferStatus(int)), SLOT(onBuffering(int)));
  connect(mediaObject, SIGNAL(currentSourceChanged(Phonon::MediaSource)),
          SLOT(onPhononCurrentSourceChanged(Phonon::MediaSource)));
  connect(mediaObject, SIGNAL(aboutToFinish()),
          SLOT(phononEnqueue())); // this fires when the whole queue is about to
                                  // finish
  ...
  connect(audioOutput, SIGNAL(outputDeviceChanged(Phonon::AudioOutputDevice)),
          SLOT(onOutputDeviceChanged(Phonon::AudioOutputDevice)));
  connect(audioOutput, SIGNAL(volumeChanged(qreal)),
